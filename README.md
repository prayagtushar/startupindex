...
[![CI](https://github.com/prayagtushar/startupindex/actions/workflows/ci.yml/badge.svg)](https://github.com/prayagtushar/startupindex/actions/workflows/ci.yml)

**[Live demo](https://isra.prayagtushar.xyz)** — no signup. Redeployed 2026-09-03 after an August teardown, now running on Cloud Run with `min-instances=0` and no load balancer, so it costs effectively nothing at demo traffic. A cold request pays about 30 seconds to load the two BGE models; a keep-warm ping every ten minutes covers that.
...
Live as of 2026-09-03. The API runs on Cloud Run in `asia-south1`, the web app on Vercel, and Postgres on Supabase, reached through the session pooler because Supabase now serves the direct `db.<ref>` hostname over IPv6 only.

Build the image with [`infra/cloudbuild.yaml`](infra/cloudbuild.yaml) rather than `gcloud builds submit --tag`: the Dockerfile uses `RUN --mount=type=cache`, which needs BuildKit, and the default builder does not enable it.

Deploy with `--min-instances=0` and **no load balancer**. A GCP external HTTP load balancer bills around $18 a month whether or not anyone visits, and it does not scale to zero.
